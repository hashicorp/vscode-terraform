...
suite("Index Tests", () => {
  suite("IndexGroup Tests", () => {
    test("create", () => {
      let group = new IndexGroup(Uri.file("directory"));

      assert.equal(group.indices("ALL_FILES").length, 0);
    });

    test("create from index", () => {
      let [index, diagnostic] = FileIndex.fromString(Uri.file("dir/file.tf"), "");
      let group = IndexGroup.createFromFileIndex(index);

      assert.equal(group.indices("ALL_FILES").length, 1);
      assert.equal(group.uri.toString(), "file:///dir");
    });

    test("add index", () => {
      let [index, diagnostic] = FileIndex.fromString(Uri.file("dir/file.tf"), 'variable "var" {}');
    ...
    test("add index fails if wrong group", () => {
      ...
      let [index2, d2] = FileIndex.fromString(Uri.file("dir2/file2.tf"), 'variable "var2" {}');
    ...
    test("delete", () => {
      ...
      let [index2, d2] = FileIndex.fromString(Uri.file("dir/file2.tf"), 'variable "var2" {}');
      group.add(index2);
