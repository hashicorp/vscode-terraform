...
- **Step 3:** To activate the extension, open any folder or VS Code workspace containing Terraform, Terraform Stacks, Terraform Search, Terraform Policy or Terraform Policy Test files. Once activated, the Terraform language indicator will appear in the bottom right corner of the window.
...
This extension provides Terraform, Terraform Stacks, Terraform Search, Terraform Policy and Terraform Policy Test language features. For most features Terraform Stacks, Terraform Search, Terraform Policy, Terraform Policy Test support is implied, specific functionality is called out where approprite.
...
- [Syntax highlighting](#syntax-highlighting) Highlighting syntax for Terraform 0.12 to 1.X, Terraform Stacks, Terraform Search, Terraform Policy and Terraform Policy Test
...
The Terraform Extension for VS Code provides IntelliSense support for Terraform, Terraform Stacks, Terraform Search, Terraform Policy and Terraform Policy Test.
...
For Terraform Policy Test files, the extension provides auto-completion for policy specific blocks and attributes, allowing policy authors to define test cases in HCL, mocking resource definitions, data sources, and provider configurations. This framework helps you ensure that all your policies perform the evaluations you expect, leading to higher confidence in the policy's behavior and enabling robust unit and integration testing.

This means that you can benefit from auto-completion, parameter info, quick info, and member lists when working with Terraform, Terraform Stacks, Terraform Search, Terraform Policy and Terraform Policy Test files. Whether you're defining stacks, resources, variables, search queries or policies the extension will provide context-specific completions to help you write your code more efficiently.
...
Terraform, Terraform Stacks, Terraform Search, Terraform Policy and Terraform Policy Test configuration files are validated when opened and on change, and invalid code is marked with diagnostics.

HCL syntax is checked for e.g. missing control characters like `}`, `"` or others in the wrong place.

![](docs/validation-rule-hcl.png)

Enhanced validation of selected Terraform language constructs in both `*.tf` and `*.tfvars` files based on detected Terraform version and provider versions is also provided. This validation also extends to language constructs for Terraform Search (`*.tfquery.hcl`), Terraform Stacks (`*.tfcomponent.hcl`, `*.tfstack.hcl`, and `*.tfdeploy.hcl`), Terraform Policy (`*.policy.hcl`) and Terraform Policy Test (`*.policytest.hcl`).
...
Terraform Policy & Terraform Policy syntax highlighting recognizes language constructs from Terraform version 1.16 to 1.X.
...
While editing, you can right-click different identifiers in Terraform, Terraform Stacks, Terraform Search, Terraform Policy and Terraform Policy Test files to take advantage of several convenient commands:
...
This works for Terraform, Terraform Stacks, Terraform Search, Terraform Policy and Terraform Policy Test files.
